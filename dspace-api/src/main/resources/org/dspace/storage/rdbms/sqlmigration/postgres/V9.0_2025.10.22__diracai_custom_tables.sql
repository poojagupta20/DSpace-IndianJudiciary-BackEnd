...
CREATE TABLE IF NOT EXISTS file_hash_record (
    ...
    ackid VARCHAR(255),
    zipstatus VARCHAR(255),
    postresponse TEXT,
    poststatus VARCHAR(255),
    getcheckresponse TEXT,
    getcheckstatus VARCHAR(255),
    filecount INTEGER DEFAULT 0,
    uploaddate TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    batchname VARCHAR(255) DEFAULT 'N/A',
    casetype VARCHAR(255) DEFAULT 'Unknown',
    caseno VARCHAR(255) DEFAULT 'N/A',
    status VARCHAR(255) DEFAULT 'Pending',
    cinonumber VARCHAR(255) DEFAULT 'N/A',
...
CREATE TABLE IF NOT EXISTS bulk_upload_request (
    ...
    status TEXT NOT NULL CHECK (status IN ('CLAIMED', 'APPROVED', 'REJECTED', 'PENDING')),
    uploadeddate TIMESTAMP NOT NULL,
    revieweddate TIMESTAMP NOT NULL
);

CREATE INDEX IF NOT EXISTS idx_bulk_upload_request_uploader ON bulk_upload_request(uploaderid);
CREATE INDEX IF NOT EXISTS idx_bulk_upload_request_reviewer ON bulk_upload_request(reviewerid);
CREATE INDEX IF NOT EXISTS idx_bulk_upload_request_status ON bulk_upload_request(status);
